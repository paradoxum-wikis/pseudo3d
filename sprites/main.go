...
func init() {
	...
	flag.IntVar(&internal.SizeTarget, "size", 512, "Output size of the square frames (for example, 300 for 300x300)")
	flag.BoolVar(&internal.SizeOne, "size-one", false, "If set, resize one-shot output to the same size as --size")
	flag.Float64Var(&internal.Threshold, "threshold-bg", 7000.0, "Tolerance threshold for background removal (higher = more aggressive); also used as max value for range mode")
	flag.BoolVar(&internal.SkipBgRemoval, "skip-bg", false, "Skip chroma key background removal completely")
	flag.StringVar(&internal.ModeBg, "mode-bg", "range", "Background removal mode: hard (1 color) or range (a range of colors)")
	flag.Float64Var(&internal.ThresholdMin, "threshold-bg-min", 1000.0, "Minimum threshold for range removal (used when mode-bg=range)")
